...
파이썬 2.7.6 버전으로
automata1_baadchim_first.py 를 run하면 받침우선 오토마타를,
automata1_chosung_first.py 를 run하면 초성우선 오토마타를 실행합니다.
...
이 때 고정된 값인 states, vocabulary, initialState는 각 py파일의 맨 처음에 정의되어 있습니다.
vocabulary와 initialState는 동일하고, 초성우선 형식은 받침우선 형식에 하나의 state가 추가된('X') states를 가집니다.
...
만약, 아무 값도 입력하지 않고 엔터를 치면 기본 인풋으로 문자열 'ㅁㅜㄹㄲㅡㄹㅎㄴㅡㄴㄷㅏ' 가 들어갑니다.
...
이 때 편의를 위해 한 스테이트에서 다른 스테이트로 가는 인풋이 많은 경우 (ex: S에서 어떤 초성을 받아도 V로 가는 경우)
각 인풋에 대한 줄을 모두 만들지 않고 스페이스바로 나누어진 3부분 중 가운데 부분에 @ 기호를 이용해 구분해 썼습니다. (ex: S ㄱ@ㄴ@...@ㅉ V)
...
또한 인풋이 들어오지 않고 엔터만 쳤을 경우 기본 인풋 'ㅁㅜㄹㄲㅡㄹㅎㄴㅡㄴㄷㅏ' 가 들어가도록 수정하였습니다.
