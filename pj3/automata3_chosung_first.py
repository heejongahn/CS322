...
def CheckFinal(currentString, meaningless) :
    length = len(currentString)

    rest = currentString[:length-3]
    currentWord = currentString[length-3:]
    lastWord = rest[length-6:length-3]
    lastBaadChimIndex = (ord(lastWord.decode('utf-8')) - BASE) % JUNGSUNG

    if ord(currentWord.decode('utf-8')) <= BASE:
        if lastBaadChimIndex == 0:
            return MoEumtoBaadChim(rest, currentWord)
        else:
            return BaadChimtoBaadChim(rest, currentWord)
    else:
        return currentString
